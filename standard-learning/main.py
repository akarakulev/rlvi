...
roboto_font = fm.FontProperties(fname="../Roboto.ttf", size=22)
...
def test_mean():
    ...
    plt.xlabel(r"corruption level $\varepsilon$", font_properties=roboto_font)
    # plt.ylabel("$Average\ relative\ error$", fontsize=22)
    plt.ylabel("Mean\navg. relative error", font_properties=roboto_font)
...
def test_mean_fixed_eps():
    ...
    plt.xticks(range(1, len(errors_to_plot) + 1), errors_to_plot.keys(), font_properties=roboto_font)
    plt.ylabel("Mean\nrelative error", font_properties=roboto_font)
    plt.ylim(0, 0.12)
...
def test_linear_regression():
    ...
    plt.xlabel(r"corruption level, $\varepsilon$", font_properties=roboto_font)#, labelpad=10)
    plt.ylabel("LinReg\navg. relative error", font_properties=roboto_font)
    plt.xticks(np.arange(0, 0.5, 0.1), fontsize=20)
    plt.xlim(0, 0.4)
    plt.yticks(np.arange(0.010, 0.06, 0.01), fontsize=20)
    plt.ylim(0.015, 0.06)
...
def test_linear_regression_fixed_eps():
    ...
    plt.ylabel("LinReg\nrelative error", font_properties=roboto_font)
    plt.ylim(0, 0.15)
    plt.yticks(np.arange(0, 0.16, 0.05), font_properties=roboto_font)
...
def test_logistic_regression():
    ...
    plt.xlabel(r"corruption level $\varepsilon$", font_properties=roboto_font)
    plt.ylabel("LogReg\navg. angle in degree", font_properties=roboto_font)
...
def test_logistic_regression_fixed_eps():
    ...
    for _ in range(MC_runs):
        ...
        # theta_ml, _ = utils.mm_log_reg(X, y)
        theta_sever = sever.logistic_regression(X, y, eps=0.1)
        theta_rrm = rrm.logistic_regression(X, y, eps=0.1)
        theta_rlvi = rlvi.logistic_regression(X, y)
        ml_angles.append(
    ...
    for i, errors in enumerate(errors_to_plot.values()):
        plt.boxplot(errors, positions=[i + 1], **box_plot_props)
    plt.xticks(range(1, len(errors_to_plot) + 1), errors_to_plot.keys(), font_properties=roboto_font)
    plt.ylabel("LogReg\nangle in degree", font_properties=roboto_font)
    plt.ylim(0, 30)
...
def test_pca():
    ...
    # Original benchmark in Osama et al. (2020) used the initial guess for RRM
    # so do we –– but for a fair comparison we use it for all methods: SEVER, RRM, and RLVI
    theta_init = np.array([0.1, 1])
    theta_init /= np.linalg.norm(theta_init)
    for _ in range(MC_runs):
        sample = generate_data_pca(size=40, eps=0.2)
        theta_ml, _ = utils.pca(sample)
    ...
    for i, errors in enumerate(errors_to_plot.values()):
        plt.boxplot(errors, positions=[i + 1], **box_plot_props)
    plt.xticks(range(1, len(errors_to_plot) + 1), errors_to_plot.keys(), font_properties=roboto_font)
    plt.ylabel("PCA\nmisalignment error", font_properties=roboto_font)
    plt.ylim(0, 0.061)
...
def test_covariance():
    ...
    for _ in range(MC_runs):
        ...
        cov_rlvi = rlvi.covariance(samples)
